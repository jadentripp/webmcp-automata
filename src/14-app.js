...
(function () {
  ...
  class App {
    constructor() {
      ...
      this.drawing = false;
      this.drawValue = 1;
      this.eraseMode = false; // toggle for touch users (no shift key)
    ...
    _bindUI() {
      ...
      // erase toggle (mainly for touch, where there is no shift key)
      const eb = $('btn-erase');
      if (eb) eb.addEventListener('click', () => {
        this.eraseMode = !this.eraseMode;
        eb.classList.toggle('armed', this.eraseMode);
        this.log('you', 'erase mode ' + (this.eraseMode ? 'on' : 'off'));
      });

      // canvas drawing (pointer events: mouse + touch + pen)
      this.canvas.addEventListener('pointerdown', (e) => this._onPointerDown(e));
      window.addEventListener('pointerup', () => { this.drawing = false; this._lastDrawCell = null; });
      window.addEventListener('pointercancel', () => { this.drawing = false; this._lastDrawCell = null; });
    ...
    _onPointerDown(e) {
      ...
      this.drawing = true;
      this.drawValue = (this.eraseMode || e.shiftKey || e.button === 2) ? 0 : 1;
